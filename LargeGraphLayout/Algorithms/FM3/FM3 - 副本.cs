...
namespace LargeGraphLayout.Algorithms.FM3
{
    public class FM3
    {
        ...
        public FM3(Graph largestGraph, FM3Config config)
        {
            ...
            while (true)
            {
                var graph = this.Coarsening(graphArr.Last());
                if (graph.IdSet.Count < this.Config.LayerMinimalNodeCount)
                    break;
                graphArr.Add(graph);
                Trace.WriteLine($"Layout: {graph.IdSet.Count}");
            }
            
            var biggestLen = Math.Sqrt(largestGraph.IdSet.Count);
            var smallestSet = graphArr.Last().IdSet;
            var count = smallestSet.Count;
            this.XCoordArr = new Dictionary<int, double>(count);
            this.YCoordArr = new Dictionary<int, double>(count);
            Dictionary<int, double> xArr = this.XCoordArr, yArr = this.YCoordArr;
            Random random = new Random(3145);
            for (var j = 0; j < count; j++)
            {
                var id = smallestSet[j];
                xArr.Add(id, random.NextDouble() * biggestLen);
                yArr.Add(id, random.NextDouble() * biggestLen);
            }
            
            var iterationsArr = new List<int>();
            var maxI = graphArr.Count- 1;
            const double kValue = 1, tDecrease = 0.95, forceDecreaseBound = 0.99;
            var watch = new Stopwatch();
            watch.Start();
            var iterRange = maxIteration - minIteration;
            var tValueArr = new List<double>(maxI);
            biggestLen = largestGraph.IdSet.Count;
            for (var k = 0; k <= maxI; k ++)
            {
                iterationsArr.Add(iterRange * (k / maxI) + minIteration);
                tValueArr.Add(biggestLen * (k / maxI));
            }
            List<Graph> graphLayers = new List<Graph>();
            Trace.WriteLine($"{DateTime.Now.ToString("T")}\tComputing started");
            var globalForceArr = new List<double>();
            var kdTree = new TreeNode();
            for (var i = maxI; i >= 2; i ++)
            {
                CurrLayer = i;
                ...
                for (var currIter = 0; currIter < iterations; currIter ++)
                {
                    if (currIter < 3 || currIter % 20 == 0)
                        kdTree = this.GetKDTree(currGraph, 100);
                    globalForceArr.Add(MoveVertex(currGraph, kdTree, tValue, useK));
                    if (globalForceArr.Count == 3)
                    {
                        if (globalForceArr[2] / globalForceArr[0] > forceDecreaseBound)
                            tValue *= tDecrease;
                        globalForceArr.RemoveAt(0);
                    }
                    if (currIter % (iterations / 10) == 0)
                        Trace.WriteLine(DateTime.Now.ToString("T") + $"\tCurrent Iter: {currIter}.\t");
                }
                Trace.WriteLine($"{DateTime.Now.ToString("T")}\tLayout: {i}\tIteration times: {iterations}");
                graphLayers.Add(currGraph.PersistentGraph(this.XCoordArr, this.YCoordArr));

                if (i <= 0) continue;
                this.Interpolation(graphArr[i - 1], 50);
                graphArr.RemoveAt(i);
            }
            //graphLayers.Add(graphArr[0].PersistentGraph(this.XCoordArr, this.YCoordArr));
            this.GraphLayers = graphLayers;
            watch.Stop();
            Trace.WriteLine($"Computing:\t{watch.Elapsed}");
        }
        ...
        private Graph Coarsening(Graph graph)
        {
            ...
            List<int> list = new List<int>(len1);
            for (var i = 0; i < len1; i++)
                list.Add(i);
            //for (var j = 0; j < len1; j ++)
            //{
            //    var id1 = random.Next(len1 - 1);
            //    var id2 = random.Next(len1 - 1);
            //    var t = list[id1];
            //    list[id1] = list[id2];
            //    list[id2] = t;
            //}
            //var list = GetRandomOrderItems(idSet, random);
            for (var j = 0; j < len1; j++)
            {
                var ddd = list[j];

                var currId = idSet[ddd];
                if (deleteSet.Contains(currId))
                    continue;

                var adjSet = adjList[ddd];
                if (adjSet.Count == 0)
                {
                ...
                newIdSet.Add(currId);
                chargeSet.Add(getChargeFunc(ddd));
            ...
            Dictionary<int, int> parentIdSet)
        {
            ...
            for (var j = 0; j < idCount; j++)
            {
                ...
                var searchHistory = new Dictionary<int, int>();
                idQueue.Add(newId);
                distanceQueue.Add(0);
                searchHistory[newId] = 1;
                ...
                while (idQueue.Count > 0)
                {
                    ...
                    for (var k = 0; k < len; k++)
                    {
                        ...
                        if (searchHistory.ContainsKey(adjId))
                            continue;

                        searchHistory[adjId] = 1;
                        var newDistance = currDistance + getWeightFunc(oldIndex, k);
                        if (parentIdSet[adjId] == adjId)
                        {
                            if (!newAdjMap.ContainsKey(adjId))
                            {
                                //newAdjSet[adjCount] = adjId;
                                newAdjSet.Add(adjId);
                                //newWeightSet[adjCount] = newDistance;
        ...
        private TreeNode GetKDTree(Graph graph, int shift)
        {
            ...
            {
                var id = idSet[j];
                var xBin = Utils.ToString(xArr[id]*shift, 2);
                ...
                var yBin = Utils.ToString(yArr[id]*shift, 2);
            ...
            IReadOnlyDictionary<int, int> idToIndex, Func<int, double> getChargeFunc)
        {
            ...
            for (var k = 0; k < 4; k++)
            {
                ...
                akValueArr.Add(akValue.Divide((k + 1)));
            }
                    ...
                    if (distance2 > node.Radius * node.Radius)
                    {
                        ...
                        if (useK)
                        {
                            foreach (var akValue in akValueArr)
                            {
                                ...
                                //Trace.WriteLine("Prev: " + ratio.Norm());
            ...
            Dictionary<int, double> yArr)
        {
            ...
            return attraction.Multiply(this.Config.Epsilon1);//.Normalize(100);
        }
        ...
        private double MoveVertex(Graph graph, TreeNode kdTree, double t, bool useK)
        {
            ...
            //foreach (var id in idSet)
            {
                ...
                newXArr[id] += moveStep.Real;
                newYArr[id] += moveStep.Imaginary;
            });
